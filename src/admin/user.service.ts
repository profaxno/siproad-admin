...
export class UserService {
  ...
  updateUser(dto: UserDto): Promise<UserDto> {
    ...
    .then( (companyList: Company[]) => {
      ...
      return this.findUsersByParams({}, searchDto)
  ...
  createUser(dto: UserDto): Promise<UserDto> {
    ...
    .then( (companyList: Company[]) => {
      ...
      // * find user
      const searchDto: SearchDto = new SearchDto(undefined, [dto.fullName]);
        
      return this.findUsersByParams({}, searchDto, company.id)
      .then( (entityList: User[]) => {
  ...
  findUsers(companyId: string, paginationDto: PaginationDto, searchDto: SearchDto): Promise<UserDto[]> {
    const start = performance.now();

    return this.findUsersByParams(paginationDto, searchDto, companyId)
  ...
  findOneUserByValue(companyId: string, value: string): Promise<UserDto[]> {
    ...
    return this.findUsersByParams({}, searchDto, companyId)
    .then( (entityList: User[]) => entityList.map( (entity: User) => new UserDto(entity.company.id, entity.fullName, entity.email, entity.password, entity.id) ) )// * map entities to DTOs
    .then( (dtoList: UserDto[]) => {
  ...
  private findUsersByParams(paginationDto: PaginationDto, searchDto: SearchDto, companyId?: string): Promise<User[]> {
    ...
    if(searchDto.searchList) {
      return this.userRepository.find({
        ...
        where: {
          ...
          fullName: In(searchDto.searchList),
          active: true,
        }
      })
    }
